...
//   const benefitsText = `Estimated annual water savings: ~₹${annualSavings} (based on ${usableHarvestM3} m³ usable harvest). Non-monetary benefits: improved water security, reduced dependency on tanker water, recharge to local aquifer.`;
...
//     // --- Personal Details ---
//     const personalDetails = [`Name: ${fullName || "—"}`, `Dwellers: ${dwellers || "—"}`, `Location: ${locationText || "—"}`];
...
//       data: {
//         labels: months,
//         datasets: [{ label: "Monthly Rainfall (mm)", data: monthlyRainfall, backgroundColor: "rgba(54, 162, 235, 0.6)" }]
//       },
//       options: { responsive: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
//     });

//     await new Promise(res => setTimeout(res, 100)); // wait for chart render
//     const imgData = canvas.toDataURL("image/png");
//     doc.text("Annual Rainfall Overview:", margin, y); y += lineSpacing;
//     doc.addImage(imgData, "PNG", margin, y, pageWidth - margin * 2, 60);
//     y += 65;
